...
| Q-507 | **Quelle horloge fait foi pour « la date du jour » ?** Le cabinet porte un fuseau (`practices.timezone`, défaut `Europe/Paris`, migration `0001`), lu par l'application depuis le 2026-09-13 — pour la seule fiche patient. Ailleurs, l'application calcule la date en **UTC** (`toISOString().slice(0, 10)`, 19 endroits : dates proposées par défaut pour une émission de facture, un règlement, un consentement accordé ou retiré, une note, un bilan, et la navigation de l'agenda) et la base par `current_date`, **dans le fuseau de sa session** (21 occurrences dans les migrations, dont certaines dans des versions de fonctions redéfinies depuis). **Les deux côtés ne peuvent pas être corrigés séparément** : cinq fonctions d'émission refusent une date postérieure à `current_date` (`0016`, `0022` à `0025`) — passer l'application seule à l'heure du cabinet ferait ÉCHOUER une émission entre minuit et deux heures du matin, au lieu de la décaler. S'y ajoutent `instant()` de l'agenda, qui interprète l'heure saisie dans le fuseau du PROCESSUS serveur (démontré : 14 h 30 saisies deviennent 16 h 30 à Paris sous un processus UTC, invisible sur la machine de développement réglée sur Paris), et trois fonctions SQL qui figent `'Europe/Paris'` sans lire la colonne. **Mesuré le 2026-09-14 : la base de production tourne en `UTC`.** Ce qui reste côté base n'est donc pas théorique : un courrier, une synthèse, un écrit de fin ou un écrit pour un tiers remis entre minuit et deux heures du matin à Paris reçoit la date de la veille. Le fuseau d'exécution des fonctions Vercel, qui gouverne `instant()` avant sa correction, n'a pas été lu. Touche des données déjà enregistrées : à trancher avant correction. **Depuis le 2026-09-14, neuf des dix fichiers de la dette côté application sont corrigés, agenda compris ; la date proposée pour signer une attestation l'est aussi, depuis que `0026` — appliquée en production le 2026-09-14, empreintes vérifiées — aligne la garde de `0016` sur le jour du cabinet. **La dette côté application est soldée.** `lib/dates.architecture.test.mts` suit ce dernier fichier et interdit toute nouvelle occurrence.** Côté base, restent : les quatre écrits cliniques (`0022` à `0025`), qui prennent `current_date` par défaut et le comparent ; les défauts de colonnes `written_on` et `received_on` ; `ended_on` à l'archivage ; et la vue `realised_sessions`, qui fige `'Europe/Paris'`. **Inventaire établi avec une seconde session de travail, revérifié par lecture sauf mention** — *âge calculé sur un instant, donc dans le fuseau du processus* : liste des dossiers (`patients/page.tsx`, `formatAgeAt(p.birth_date, aujourdhui)` avec `aujourdhui = new Date()`), âge du formulaire de nouveau bilan ; *date par défaut proposée puis enregistrée* : consentement accordé, note, charges, émission et règlement d'une pièce, émission d'une attestation — cette dernière soumise au refus de `0016` ; *écriture serveur sans affichage* : `valid_to`, `written_on`, `granted_on`, `withdrawn_on`, `received_on`, `validated_on` ; *date d'expiration d'un lien* calculée en UTC puis reprise dans le message de transmission ; *périodes* : `resoudrePeriode` lit `getFullYear`, `periodeParDefaut` des synthèses, `getJourneeResume`. **Déduit du code, non revérifié** : le 1er janvier à 0 h 30, une facture émise à la date proposée serait datée du 31 décembre et numérotée dans la série de l'année close, la série suivant l'année de la date d'émission (`0009`) ; l'agenda construit son jour de référence à minuit local côté serveur puis le relit en UTC, ce qui ne serait cohérent que sous un serveur UTC — la navigation jour par jour pourrait sauter ou répéter un jour en développement. **Rapporté, non revérifié ici** : la base PostgreSQL LOCALE tourne en `Europe/Paris` ; les contrôles SQL locaux évaluent donc `current_date` à l'heure de Paris, pas comme une base réglée en UTC. | Technique + exploitant |
