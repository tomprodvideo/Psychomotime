...
| A-11 | AUDIT | L'abonnement n'est contrôlé que dans 3 pages, aucune server action | Contrôle d'accès dans chaque cas d'usage serveur, pas seulement dans l'UI | L0 | `lib/auth/guard.ts`, `app.subscription_is_active`, `lib/subscription.ts` | `030_garanties_socle` § 5 | **en cours** — garde écrite et câblée sur les actions de compte ; reste à appliquer aux 21 actions métier |
