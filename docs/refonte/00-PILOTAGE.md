...
| R-03 | résolu | `next@16.2.9` portait 1 advisory critique et 6 hautes, dont deux visaient l'architecture du produit. **Corrigé** : `next@16.3.5` en dépendance depuis le commit `98de8cf`. |
| **R-09** | moyenne | Les adresses publiques du produit sont **nommées dans le code** (`lib/origine.ts`), parce qu'un lien envoyé par courriel doit repartir sur le domaine que la personne utilise et ne jamais suivre l'en-tête `x-forwarded-host`. Ajouter un domaine au produit sans l'ajouter là renverra ses visiteurs vers l'adresse canonique — le lien reste valide, mais il change de domaine. `SITE_ORIGIN` permet de trancher sans toucher au code. |
